#include "function.h"
#include <math.h>
int GetICacheEntry(int index, int set, int num)
{
    return *(ICache+index*I_cache_associativity*3+set*3+num);
}

void SetICacheEntry(int index, int set, int num, int value)
{
    *(ICache+index*I_cache_associativity*3+set*3+num) = value;
    return;
}
...
int GetDCacheEntry(int index, int set, int num)
{
    return *(DCache+index*D_cache_associativity*3+set*3+num);
}

void SetDCacheEntry(int index, int set, int num, int value)
{
    *(DCache+index*D_cache_associativity*3+set*3+num) = value;
    return;
}
...
int I_VA2PA(int VA)
{
    ...
    else // update TLB Cache
    {
        ...
        *(ITLB+4*TLB_LRU_index+3) = *(IPTE+3*LRU_index+2);
        ...
        int index, tag;
        while(PA < (*(IPTE+3*LRU_index+2)+1)*I_page_size) // Cache = index * set * {valid,MRU,tag}
        {
            index = (PA/I_cache_WordPerBlock/4) % I_cache_index;
            tag = (PA/I_cache_WordPerBlock/4) / I_cache_index;
            for(i=0 ; i<I_cache_associativity ; i++)
            {
                if(GetICacheEntry(index, i, 0)==1 && GetICacheEntry(index, i, 2)==tag)
                {
                    SetICacheEntry(index, i, 0, 0);
                }
            }
            PA++;
        }
...
{
    PA /= 4;
    fprintf(fp_t, "cycle%d PA=%d\n", cycle, PA);
    int index = (PA/I_cache_WordPerBlock)%I_cache_index, tag = (PA/I_cache_WordPerBlock)/I_cache_index, i;
    int j;
    for( i=0 ; i<I_cache_index ; i++)
    {
        for( j=0 ; j<I_cache_associativity ; j++)
        {
            fprintf(fp_t,"(set%d v=%d MRU=%d tag=%d) ", j, GetICacheEntry(i,j,0), GetICacheEntry(i,j,1), GetICacheEntry(i,j,2));
        }
        fprintf(fp_t,"\n");
    }
    for(i=0 ; i<I_cache_associativity ; i++)
...
int D_VA2PA(int VA)
{
    // DTLB = entry * {valid(0), LRU(1), VA(2), PA(3)}
    ...
    else // update TLB Cache
    {
        ...
        *(DTLB+4*TLB_LRU_index+3) = *(DPTE+3*LRU_index+2);
        ...
        {
             index = (PA/D_cache_WordPerBlock/4) % D_cache_index;
             tag = (PA/D_cache_WordPerBlock/4) / D_cache_index;
             for(i=0 ; i<D_cache_associativity ; i++)
             {
